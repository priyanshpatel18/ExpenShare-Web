				...
				<form className="addGroupForm" onSubmit={formik.handleSubmit}>
					<div className="row1">
						<label htmlFor="profile" className="profile">
							<img src={formik.values.groupProfile ? URL.createObjectURL(formik.values.groupProfile) : group} alt="avatar" className="profileImg" />
							<input
								type="file"
								name="profile"
								id="profile"
								onChange={(e) =>
									formik.setValues({
										...formik.values,
										groupProfile: e.target.files ? e.target.files[0] : null,
									})
								}
							/>
						</label>
